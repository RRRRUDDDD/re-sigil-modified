...
{
    Resource *resource = m_BookBrowser ? m_BookBrowser->GetCurrentResource() : NULL;
    ContentTab *tab = GetCurrentContentTab();
    Resource *tab_resource = tab ? tab->GetLoadedResource() : NULL;

    if (!resource || !tab || !tab_resource) {
        QMessageBox::warning(this, tr("Sigil"), tr("You cannot insert a file at this position."));
        return;
    }
    ...
    if (tab_resource->Type() == Resource::CSSResourceType &&
        ...
        if (css_tab) {
            css_tab->InsertFile(QString("url(\"%1\")").arg(relative_path));
            return;
        }
    }

    if (tab_resource->Type() == Resource::HTMLResourceType &&
        (resource->Type() == Resource::ImageResourceType ||
         resource->Type() == Resource::SVGResourceType ||
        ...
        if (flow_tab && flow_tab->InsertFileEnabled()) {
            ...
            filename = Utility::EncodeXML(filename);
...
void MainWindow::ConnectSignalsToSlots()
{
    ...
    connect(m_BookBrowser, SIGNAL(InsertFileRequest()), this, SLOT(InsertFileFromBookBrowser()));
