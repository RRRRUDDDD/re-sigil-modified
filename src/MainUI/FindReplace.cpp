...
FindReplace::FindReplace(MainWindow *main_window)
    : QWidget(main_window),
      ...
      m_menu(nullptr),
      m_CF_RestartFlag(false) // 修改：循环查找 BUG
{
    ui.setupUi(this);
...
void FindReplace::RestartClicked()
{
    ...
    m_CF_RestartFlag = true;  // 修改：循环查找 BUG，当前页面查找时重置开始标志
    ShowMessage(tr("Search will restart"));
}
...
bool FindReplace::FindInAllFiles(Searchable::Direction direction)
{
    ...
    if (!found) {
        ...
        // 改变 GetNextContainingResource 函数：避免筛选资源时匹配到当前资源并循环；当这些资源都匹配一轮后再返回 nullptr。
        if (containing_resource && current_resource != containing_resource) {
            DBG qDebug() << " .. which is " << containing_resource->GetRelativePath();

            if (containing_resource == m_StartingResource) { // 发现起始页面时，Wrap 查找进入下一轮循环。
...
Resource *FindReplace::GetNextContainingResource(Searchable::Direction direction)
{
    ...
    */
    ...
    while (true) {
        next_resource = GetNextResource(next_resource, direction);
        // 修改：调整 GetNextContainingResource 逻辑，先处理起始页面
        if (next_resource == starting_resource) {
...
{
    //QList <Resource *> resources = GetFilesToSearch();
    QList <Resource*> resources = GetFilesToSearch(true); // 修改：循环查找 BUG，GetFilesToSearch 强制返回经过筛选的搜索资源
