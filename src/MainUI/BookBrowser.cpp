...
QStringList BookBrowser::AddImagesFromFilePaths(QStringList &filepaths)
{
    ...
    foreach(const QString &filepath, filepaths) {
        ...
        if (!file_info.isFile() || !IMAGE_EXTENSIONS.contains(file_info.suffix().toLower())) {
            continue;
        }

        try {
            // A's existing ResourceAdded signal updates the OPF one resource at a time.
            Resource *resource = folderkeeper->AddContentFileToFolder(filepath, true);
            if (resource && QFileInfo(resource->GetFullPath()).isFile()) {
                bookpaths << resource->GetRelativePath();
            }
        } catch (FileDoesNotExist&) {
            // The clipboard can outlive files copied from a file manager.
        }
    }
...
QString BookBrowser::AddImageFromClipboard(const QByteArray &data, QString default_filename)
{
    if (!m_Book || data.isEmpty() ||
        !IMAGE_EXTENSIONS.contains(QFileInfo(default_filename).suffix().toLower())) {
        return QString();
    }
    ...
    Resource *resource = folderkeeper->AddContentFromQByteArray(data, default_filename);
    if (!resource) {
        return QString();
    }
