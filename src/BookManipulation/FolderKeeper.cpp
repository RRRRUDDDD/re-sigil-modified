                                               ...
                                               const QString &folderpath)
{
    ...
    // will return that this filename is now taken
    {
        ...
        if (!bookpath.isEmpty()) {
            ...
            if (!starting_dir.isEmpty() && !folder.mkpath(starting_dir)) {
                return NULL;
            }
            ...
            if (!folder_to_use.isEmpty()) {
                if (!folder.mkpath(folder_to_use)) {
                    return NULL;
                }
                                                 ...
                                                 const QString &folderpath)
{
    ...
    QString mt = mimetype;
    if (!mt.isEmpty() && MediaTypes::instance()->GetGroupFromMediaType(mt, "").isEmpty()) {
        qDebug() << "Warning: unrecognized mediatype in OPF: " << mimetype;
        mt.clear();
    }
    if (mt.isEmpty()) {
        mt = MediaTypes::instance()->GetMediaTypeFromExtension(QFileInfo(filename).suffix().toLower(), mimetype);
    }
    ...
    {
        // Keep filename selection, file creation, and resource registration atomic
        // with respect to other resource additions.
        QMutexLocker locker(&m_AccessMutex);
        ...
        QString folder_to_use = folderpath;
        if (folder_to_use == "\\") {
            folder_to_use = GetDefaultFolderForGroup(group);
        }
        if (!folder_to_use.isEmpty()) {
            if (!folder.mkpath(folder_to_use)) {
                return NULL;
        ...
        resource->SetShortPathName(filename);
        if (!m_FileIconCache.contains(mt)) {
            m_FileIconCache[mt] = QFileIconProvider().icon(QFileInfo(new_file_path));
        }
    }
...
void FolderKeeper::DiscardResourceRegistration(Resource *resource)
{
    if (!resource) {
        return;
    }
