...
# minimal set of PyQt modules to support the plugin gui. Modern wheels add
# ABI tags to extension names (for example QtCore.abi3.so), so compare the
# module name before the first dot instead of requiring an exact filename.
...
def ignore_in_pyqt5_dirs(base, items, ignored_dirs=None):
    ...
    if ignored_dirs is None:
        ignored_dirs = {'.svn', '.bzr', '.git', 'test', 'doc', 'tests', 
                        ...
                        'qsci', 'qt', 'Qt5', 'translations', 'port_v2', 'testing', '__pycache__'}
    for name in items:
        path = os.path.join(base, name)
        if os.path.isdir(path):
            if name in ignored_dirs: # or not os.path.exists(os.path.join(path, '__init__.py')):
            ...
            if name.rpartition('.')[-1] not in ('so', 'py', 'dylib'):
                ans.append(name)
            if name.rpartition('.')[-1] == 'so' and name.split('.', 1)[0] not in PYQT_MODULE_NAMES:
                ans.append(name)
...
def main():
    ...
    pyqt_dir = os.path.abspath(os.path.join(app_dir, 'Python.framework', 'Versions',
                              pversion, 'lib', stdlib_name, 'site-packages', 'PyQt5'))
    for module_name in os.listdir(pyqt_dir):
        if module_name.endswith('.so') and module_name.split('.', 1)[0] in PYQT_MODULE_NAMES \
                and module_name.split('.', 1)[0] != 'Qt':
            module_path = os.path.join(pyqt_dir, module_name)
